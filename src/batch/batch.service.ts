...
import { StoreRowStatus, WinNumberInfoStatus } from './type'
...
export class BatchService {
  ...
  async lottoCrawling(drwNo: string) {
    const data = await axios.get(`https://www.dhlottery.co.kr/common.do?method=getLottoNumber&drwNo=${drwNo}`)
    ...
    if (data.data && data.data.returnValue === 'success') {
      ...
      try {
        ...
        lists.each(function () {
          ...
          const key: string[] = ['level', 'winTotAmnt', 'winUsers', 'winAmnt', 'continue', 'winTypeText']

          for (let i = 0; i < key.length; i++) {
            if (key[i] === 'continue') continue
            if (key[i] === 'winTypeText') {
              row[key[i]] = line.eq(i).text().replace(/\s/gi, '').replace(/1등/gi, '')
            } else {
              row[key[i]] = line
                .eq(i)
                .text()
  ...
  async storeCrawling(drwNo: string): Promise<object[]> {
    ...
    firstStore.push(...this.storeDataInit(first, $, ['continue', 'name', 'type', 'address']))
    ...
    const clearFirstStore = firstStore.filter((v: StoreRowStatus) => v.name !== '')
  ...
  async secondStoreMoreCrawling(drwNo: string, pageNo: string): Promise<object[]> {
    ...
    const secondStore: object[] = []
    secondStore.push(...this.storeDataInit(lists, $, ['continue', 'name', 'address']))
